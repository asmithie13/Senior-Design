...
struct ContentView: View {
    ...
    func endGame() {
        ...
        if useTestBench {
            ...
            // Send a special move "*" as plain text to indicate reset if needed
    ...
    func sendMove() {
        ...
        let moveToSend = lastMove
        lastMove = ""
        
        sendDataToServer(move: moveToSend)
    }
    ...
    func startNewListeningSession() {
        ...
        speechRecognizer.startListening { [self] result in
            ...
            if result == "CLEAR" {
                ...
                // Process numeric move input: extract the first 4 digits and use them directly
                let filteredMove = result.filter { $0.isNumber }.prefix(4)
                if filteredMove.count == 4 {
                    let moveString = String(filteredMove)
                    DispatchQueue.main.async {
                        self.lastMove = moveString
                    }
                }
            }
        }
    ...
    // Sends move data to server or test bench as plain text
    func sendDataToServer(move: String) {
        if useTestBench {
            handleTestResponse(move: move)
            return
        }
        
        guard let url = URL(string: "http://\(serverIP):5000/sendData") else { return }
    ...
    // Handles test bench responses with a plain move string
    private func handleTestResponse(move: String) {
        let mockResponse = TestBench.shared.getMockResponse(for: move)
...
struct MoveEntry: Identifiable {
    let id = UUID()
    var move: String    // Move notation (e.g., "1234")
