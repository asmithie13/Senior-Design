...
struct ContentView: View {
    ...
    func sendMove() {
        // We already have a 4-digit string in lastMove.
        // Send exactly what's stored in lastMove.
        ...
        let dataToSend: [String: Any] = ["move": moveToSend]
    ...
    func restartListening() {
        ...
        speechRecognizer.startListening { result in
            // Convert recognized text to uppercase to detect commands easily
            let upperText = result.uppercased()
            
            if upperText.contains("CLEAR") {
                DispatchQueue.main.async {
                    clearLastMove()
                }
            } else if upperText.contains("SEND") {
                DispatchQueue.main.async {
                    sendMove()
                }
                ...
                // Extract 4 digits only
                let filteredMove = result.filter { $0.isNumber }.prefix(4)
                if filteredMove.count == 4 {
                    DispatchQueue.main.async {
                        // Store the 4-digit string in lastMove
                        lastMove = String(filteredMove)
                    }
                }
            }
        }
    ...
    func getDataFromServer() {
        guard let url = URL(string: "http://172.20.10.4:5000/sendData") else {
            return
        }
    ...
    func sendDataToServer(data: [String: Any]) {
        // Example: data might be ["move": "1112"]
        guard let url = URL(string: "http://172.20.10.4:5000/sendData") else {
            return
        }
        ...
        // Instead of JSON, just send the raw string:
        let moveString = data["move"] as? String ?? ""
        request.setValue("text/plain", forHTTPHeaderField: "Content-Type")
        request.httpBody = moveString.data(using: .utf8)
